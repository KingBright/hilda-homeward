Homeward.define("engine/runtime", ["engine/action-track", "content/choreography", "art/performer", "art/chapter-life", "art", "story", "engine/state", "engine/audio", "engine/dom", "engine/storage", "content/items", "engine/camera"], ({ActionTrack}, Choreography, Performer, ChapterLife, Art, Story, {fresh,validate,boolFlags,numericFlags,sketches}, Soundscape, {patchSvg}, SaveStore, Items, {focusFrame}) => {
...
function updateActors(){
 ...
 p.setAttribute('transform',`translate(${hero.x.toFixed(1)} ${hero.y.toFixed(1)})`);
 p.setAttribute('class',(pendingMove?'walking ':'')+(activeWork?'working ':''));
 $('heroFacing').setAttribute('transform',`scale(${hero.face} 1)`);
 const t=$('twigPosition');if(t)t.setAttribute('transform',`translate(${petX.toFixed(1)} ${(ground(petX,s)+1).toFixed(1)}) scale(${hero.face} 1)`);
 ...
 Performer.paint($('performer'),{...hero,time:elapsed,distance:walkDistance,moving:!!pendingMove,reduced:s.settings.reduced,action:activeWork,selected});
...
function sceneDrag(down,payload){
 ...
 if(h.drag==='ladder'){if(down.oldLadder===undefined)delete s.f.ladderX;else s.f.ladderX=down.oldLadder;}
 if(h.drag==='beacon')s.beaconAngle=down.oldAngle;
...
function work(ms,label,after){
 if(activeWork)return;
 pendingMove=null;
 const ctx=currentInteraction??{scene:s.scene,id:'work',item:null};
 activeWork=new ActionTrack(s.settings.reduced?Math.min(ms,300):ms,Choreography.score(ctx.scene,ctx.id,s,ctx.item));
 activeWork.after=after;activeWork.request=sceneRequest;
 ui.game.classList.toggle('performing',[1,2,4].includes(s.scene));
...
function openHelp(){showPanel(`<div class="eyebrow">THE WORLD IS THE INTERFACE</div><h2>用好奇心来操作</h2><h3>在场景里行动</h3><p>点击地面，希尔达会走过去。点击人物、门、物件，她会走近后互动。拾取、拉绳、转轮和挑杆都有接触动作；完成前离开或重载，不会提前消耗道具。手记只在你打开时出现，没有常驻任务列表。</p><h3>物件与机关</h3><p>打开背包，选择一件物品，再点场景目标；也可以直接拖过去。选中物品后背包会收起，避免遮挡场景；再打开背包，点另一件物品可以尝试组合。雨棚绳圈、档案梯子、配重和信标均可拖动，也都有点击或键盘操作。修桥要与大卫分工；清滤网前先关闭进水闸。检修顺序写在现场铭牌上。</p><h3>看清整个世界</h3><p>手机建议横屏。竖屏可在空白处左右拖动画面，或点击边缘箭头；超宽屏可上下拖动画面查看高处。点击眼睛按钮，或按 H，会短暂标出能互动的对象。背包物品不需要丢弃。</p><h3>键盘和节奏</h3><p>Tab 选择对象，Enter / 空格互动；方向键移动；I 打开背包，J 打开手记，H 显示线索对象，F 切换全屏，Esc 暂停。对话可点“继续”或按空格。浏览器不支持系统全屏时，游戏仍铺满网页视口。</p><p>只有屋顶救援有风暴压力。失败会回到该场景的安全检查点，不会死亡或丢失前面章节。开启舒缓模式后不计时，剧情和结局相同。声音不是任何谜题的必要条件。</p><p class="fineprint">这是《希尔达》的非官方同人互动故事。剧情与代码矢量画面为本版本原创，不包含动画截图、配音或原声音乐。灵感参考的是场景式点触冒险的交互，不是《机械迷城》的素材或故事。</p><div class="btnrow"><button class="panelBtn primary" id="helpBack">回到冒险</button></div>`,'help');$('helpBack').onclick=closePanel;}
