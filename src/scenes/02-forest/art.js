...
b+=f.bridge?rep(16,i=>{let x=551+i*32,y=706+Math.sin(i/15*Math.PI)*21-i*1.1;return path(`M${x} ${y}l26-2 14 56-27 2Z`,'#a2956e','#3d5f54',2);}):rep(6,i=>group(551+i*17,711+i*22,1,`<g transform="rotate(${i*13})">${rect(-11,0,44,67,'#877f5f',3,'#385951',2)}</g>`));
b+=group(481,651,1,`${path('M-26 87-15-30H20L34 87Z','#6f7960','#31544f',3)}${wheel(0,0,43,(f.bridge?120:0))}`)+path('M1095 611v-72q-9-28-21-2','none','#a6b4a0',9)+(!f.ropeTaken?group(330,728,.85,itemArt('rope').replace(/<svg[^>]*>|<\/svg>/g,'')):'');
b+=path(f.branchBent?'M220 520Q370 560 541 615':'M220 520Q370 430 541 460','none','#284f47',7)+path(f.branchBent?'M541 615v25':'M541 460v36','none','#598572',3)+(f.branchBent?path('M541 640L495 765','none','#c8b17e',3):'')+(!f.hookTaken?group(541,f.branchBent?642:496,.83,itemArt('hook').replace(/<svg[^>]*>|<\/svg>/g,'')):'');
