...
b+=path('M-20 782q438-78 699 15t988-13V910H-20Z','url(#river)')+waterLines(796,45)+path('M67 731h411l94 73h712l86-62h230','none','#253f42',33)+path('M67 729h411l94 71h712l86-62h230','none','#809884',21)+path('M70 711v57m360-57v66M598 776v54m634-58v53m217-94v57','none','#2d4e4b',10);
b+=path('M226 192V399H640V570H940V304h337V148','none','#213f46',43)+path('M226 192V399H640V570H940V304h337V148','none','#647e73',29)+path('M226 192V399H640V570H940V304h337V148','none','#b4b58c',3,'opacity=".7"');
b+=group(453,514,1,`${rect(-50,-68,100,133,'#294b4e',15,'#8a9f85',5)}${rep(5,i=>path(`M${-30+i*15}-54v107`,'none','#9aa88a',4))}${!f.filter?rep(12,i=>group(-34+rnd()*66,-44+rnd()*75,.43+rnd()*.4,path('M0 0q-40-25-8-39 35 0 8 39Z',i%2?'#d1b172':'#8d9a69','#476450',2))):''}${!f.poleTaken?path('M-30 38 34-44','none','#c8b17e',5)+circle(34,-44,4,'#ba8c56'):''}${path('M-60-80H60M-60 78H60','none','#486961',8)}`);
b+=group(1128,496,1,`${rect(-84,-110,168,203,'#415f57',17,'#a2ab83',4)}${circle(0,-28,57,'#d2c59b','#274e4b',7)}${rep(9,i=>path(`M${Math.sin((i-4)*.3)*41} ${-28-Math.cos((i-4)*.3)*41}l${Math.sin((i-4)*.3)*8} ${-Math.cos((i-4)*.3)*8}`,'none','#526e59',2))}${path(`M0-28 ${Math.sin((f.pump?1.1:-1))*42} ${-28-Math.cos(f.pump?1.1:-1)*42}`,'none','#a2674c',4)}${circle(0,-28,7,'#33564f')}${text(0,66,f.pump?'回　流':'旁　路',14,'#cdd0a9')}`);
const pts=[[640,400],[640,570],[940,570]],network=Hydraulics.trace(vs,!!f.filter);
let flow=f.filter?'M226 192V399H587':'';
pts.forEach(([x,y],i)=>{
 b+=circle(x,y,53,'#254b50','#a0aa88',4)+circle(x,y,45,'#3c6863');
 b+=group(x,y,1,`<g transform="rotate(${vs[i]*90})">${path('M0-42V0H42','none','#1b4148',25)}${path('M0-42V0H42','none','#a0b5a0',14)}${path('M0-42V0H42','none','#d8d3a6',2)}${circle(0,0,7,'#d4c393','#355d53',2)}</g>`);
 if(network.wet.includes(i)){
  b+=group(x,y,1,`<g transform="rotate(${vs[i]*90})">${path('M0-40V0H40','none','#b5ead4',4,'stroke-dasharray="10 6" class="flowline"')}</g>`);
