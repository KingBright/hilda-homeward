...
def main(browser):
    ...
    check('Aligned ladder still needs its wheel brake',not flags(p,'planTaken'))
    act(p,'ladderBrake');act(p,'book')
    ...
    check('Lever cannot lift until David braces its fulcrum',not flags(p,'fridaSafe') and 'pole' in state(p)['inventory'])
    act(p,'david');use(p,'pole','frida')
    drag_object(p,'beacon',target=(1096,501))
...
if __name__=='__main__':
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            exec((ROOT/'tests/setpiece-regressions.py').read_text(),globals())
            run_setpiece_regressions(browser)
